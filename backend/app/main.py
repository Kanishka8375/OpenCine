...
def startup_init() -> None:
    """Initialize DB schema on startup without crashing the API process."""
    try:
        Base.metadata.create_all(bind=engine)
        logger.info('Database schema initialization complete')
    except SQLAlchemyError:
        logger.exception('Database initialization failed; API will start but DB-backed routes may fail until DB is reachable')
